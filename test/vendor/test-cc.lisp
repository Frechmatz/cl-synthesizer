...
(define-test test-clip-127 ()
	     (assert-equal 0 (cl-synthesizer-vendor-cc-handler::clip-127 0))
	     (assert-equal 0 (cl-synthesizer-vendor-cc-handler::clip-127 -1))
	     (assert-equal 127 (cl-synthesizer-vendor-cc-handler::clip-127 127))
	     (assert-equal 127 (cl-synthesizer-vendor-cc-handler::clip-127 128))
	     (assert-equal 5 (cl-synthesizer-vendor-cc-handler::clip-127 5)))

(defparameter *CC-HANDLER-INITIAL-CV* 50)

(defmacro with-cc-handler (&body body)
  `(let* ((vendor *CC-TEST-VENDOR*)
	   (handler (funcall #' cl-synthesizer-vendor-cc-handler:7-bit-relative
				vendor
				:ENCODER-2
				:cv-initial *CC-HANDLER-INITIAL-CV*
				:cv-min 0
				:cv-max 1270)))
     (let ((controller-number (cl-synthesizer-vendor:get-controller-number vendor :ENCODER-2))
