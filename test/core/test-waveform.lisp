...
(define-test test-waveform-square-2 ()
  ...
  (let ((trigger (funcall #'cl-synthesizer-test::zero-crossing-trigger))
	...
	(trigger-count 0))
    (dotimes (i 7250)
		 (if (funcall trigger (cl-synthesizer-core:phase-square-converter (funcall phase-generator 1) :duty-cycle 0.25))
		     (setf trigger-count (+ trigger-count 1))))
    ;; 14 zero-crossings
    (assert-equal 14 trigger-count)))
...
(define-test test-waveform-square-3 ()
  ...
  (let ((trigger (funcall #'cl-synthesizer-test::zero-crossing-trigger))
	...
	(trigger-count 0))
    (dotimes (i 7250)
		 (if (funcall trigger (cl-synthesizer-core:phase-square-converter (funcall phase-generator 1) :duty-cycle 1.0))
		     (setf trigger-count (+ trigger-count 1))))
    ;; 0 zero-crossings
...
(define-test test-waveform-square-4 ()
  ...
  (let ((trigger (funcall #'cl-synthesizer-test::zero-crossing-trigger))
	...
	(trigger-count 0))
    (dotimes (i 7250)
		 (if (funcall trigger (cl-synthesizer-core:phase-square-converter (funcall phase-generator 1) :duty-cycle 0.0))
		     (setf trigger-count (+ trigger-count 1))))
    ;; 0 zero-crossings
    (assert-equal 0 trigger-count)))
