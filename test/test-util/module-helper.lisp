...
(defun get-module-output (module socket)
  (funcall (getf (funcall (cl-synthesizer:get-outputs-fn module)) socket)))

(defun update-module (module input-args)
  (let ((sockets nil) (module-inputs (funcall (cl-synthesizer:get-inputs-fn module))))
...
(defun get-module-output-sockets (module)
  "TODO Inefficient implementation, but for now live with it"
  (let ((sockets nil))
    (cl-synthesizer-macro-util:with-property-list (funcall (cl-synthesizer:get-outputs-fn module)) socket fn
      (declare (ignore fn))
      (push socket sockets))
    sockets))
