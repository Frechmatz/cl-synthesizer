...
(defun print-vco-state (vco)
  (format t "~%VCO: f-exp: ~a f-lin: ~a f: ~a~%"
	  (funcall (getf vco :state) :exponential-frequency)
	  (funcall (getf vco :state) :linear-frequency)
	  (funcall (getf vco :state) :frequency)))
...
(define-test vco-test-frequency-clipping-exp-bottom ()
		 ...
		 (assert-true (is-approximately -12000 (funcall (getf vco :state) :frequency) 0.5)))))
...
(define-test vco-test-fm-2 ()
		 ...
		 (assert-true (is-approximately -3120 (funcall (getf vco :state) :frequency) 0.5)))))
