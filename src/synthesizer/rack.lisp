...
(defun update (rack)
  ...
  ;; (declare (optimize (debug 3) (speed 0) (space 0)))
  (if (getf rack :rack-has-shut-down)
      nil
      (progn
	...
	(labels
	  ...
	  ;; for all modules
	  (dolist (rm (getf rack :modules))
	    (update-rm rm))
	  ;; for all hooks
	  (dolist (m (getf rack :hooks))
...
(defun shutdown (rack)
  ...
  (if (not (getf rack :rack-has-shut-down))
      (progn
	(setf (getf rack :rack-has-shut-down) t)
	(dolist (rm (getf rack :modules))
